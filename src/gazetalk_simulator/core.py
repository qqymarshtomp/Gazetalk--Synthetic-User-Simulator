...
# =========================
# 3. Text suggestion providers
# =========================
DEFAULT_WORD_CONTINUATION_URL = "https://cloudapidemo.azurewebsites.net/continuations"
DEFAULT_LETTER_CONTINUATION_URL = "https://cloudapidemo.azurewebsites.net/lettercontinuations"


class SuggestionProvider:
    """Interface for word and letter suggestion backends.

    The click simulator uses this interface to obtain the same kind of
    autocomplete suggestions that a GazeTalk-style frontend would use during
    text entry. A provider receives the text typed so far and returns either
    word continuations or next-letter suggestions.
    """

    name = "base"

    def get_word_suggestions(self, prompt: str, locale: str = "en_US", limit: int = 4) -> List[str]:
        raise NotImplementedError

    def get_letter_suggestions(self, prompt: str, locale: str = "en_US", limit: int = 9) -> List[str]:
        raise NotImplementedError


class CloudAutocompleteSuggestionProvider(SuggestionProvider):
    """Suggestion provider backed by cloud autocomplete endpoints.

    The expected request body is:
        {"locale": "en_US", "prompt": "<text typed so far>"}

    The response parser is intentionally permissive because autocomplete
    services may return either a list of strings, a list of objects, or a
    dictionary containing a list under keys such as "suggestions",
    "continuations", "results", "words", or "letters".
    """
    ...
    def __init__(
        self,
        word_endpoint: str = DEFAULT_WORD_CONTINUATION_URL,
        letter_endpoint: str = DEFAULT_LETTER_CONTINUATION_URL,
        timeout: float = 5.0,
    ) -> None:
        self.word_endpoint = word_endpoint
        self.letter_endpoint = letter_endpoint
        self.timeout = timeout

    def _post_json(self, endpoint: str, prompt: str, locale: str) -> Any:
        payload = json.dumps({"locale": locale, "prompt": prompt}).encode("utf-8")
        ...
        if isinstance(payload, dict):
            for key in (
                ...
                if isinstance(value, list):
                    return value
            for key in ("text", "word", "letter", "completion", "value"):
                value = payload.get(key)
    ...
    def _item_to_text(self, item: Any) -> str:
        ...
        if isinstance(item, dict):
            for key in ("text", "word", "letter", "completion", "value", "token"):
                value = item.get(key)
                if isinstance(value, str):
                    return value
        return ""

    def _normalise_words(self, items: List[Any], limit: int) -> List[str]:
        suggestions: List[str] = []
        seen = set()
        for item in items:
            text = self._item_to_text(item).strip().lower()
    ...
    def _normalise_letters(self, items: List[Any], limit: int) -> List[str]:
        suggestions: List[str] = []
        seen = set()
        for item in items:
            text = self._item_to_text(item).strip().lower()
    ...
    def get_word_suggestions(self, prompt: str, locale: str = "en_US", limit: int = 4) -> List[str]:
        payload = self._post_json(self.word_endpoint, prompt, locale)
        return self._normalise_words(self._extract_items(payload), limit)

    def get_letter_suggestions(self, prompt: str, locale: str = "en_US", limit: int = 9) -> List[str]:
        payload = self._post_json(self.letter_endpoint, prompt, locale)
        return self._normalise_letters(self._extract_items(payload), limit)


def build_suggestion_provider(
    provider: str = "local",
    locale: str = "en_US",
    word_endpoint: Optional[str] = None,
    letter_endpoint: Optional[str] = None,
    timeout: float = 5.0,
) -> Optional[SuggestionProvider]:
    """Create a suggestion provider from CLI or environment settings.

    Returns None for the local provider. In that case, the simulator uses its
    built-in lexical-history prediction model. For "cloud", the simulator calls
    the autocomplete endpoints used by the frontend.
    """

    provider = (provider or os.environ.get("GAZETALK_SUGGESTION_PROVIDER", "local")).lower()
    if provider in {"local", "none", "off"}:
        return None
    if provider in {"cloud", "azure", "autocomplete"}:
        return CloudAutocompleteSuggestionProvider(
            word_endpoint=word_endpoint or os.environ.get("GAZETALK_WORD_CONTINUATION_URL", DEFAULT_WORD_CONTINUATION_URL),
            letter_endpoint=letter_endpoint or os.environ.get("GAZETALK_LETTER_CONTINUATION_URL", DEFAULT_LETTER_CONTINUATION_URL),
            timeout=timeout,
        )
    raise ValueError(f"Unknown suggestion provider: {provider}")


# =========================
# 4. Click and time simulator
# =========================
class GazetalkClickSimulator:
    def __init__(
        ...
        suggestion_locale: str = "en_US",
        fill_external_suggestions: bool = True,
    ):
        ...
        self.suggestion_locale = suggestion_locale
        self.fill_external_suggestions = fill_external_suggestions
    ...
    def _prompt_so_far(self, context_words: List[str], prefix: str = "") -> str:
        """Build the frontend-style prompt: text typed so far."""
        if prefix:
            return " ".join(context_words + [prefix]) if context_words else prefix
        if context_words:
            # A trailing space indicates that the next word is about to start.
            return " ".join(context_words) + " "
        return ""

    def _dedupe(self, items: List[str], limit: int) -> List[str]:
        result: List[str] = []
        seen = set()
        for item in items:
    ...
    def _recommend_words_local(self, context_words: List[str], profile: SyntheticUserProfile, prefix: str = "") -> List[str]:
    ...
    def _visible_letters_local(self, context_words: List[str], profile: SyntheticUserProfile, prefix: str = "") -> List[str]:
        prev_word = context_words[-1] if context_words else "<s>"
    ...
    def recommend_words(self, context_words: List[str], profile: SyntheticUserProfile, prefix: str = "") -> List[str]:
        """Return predicted words from the selected suggestion backend.

        If no external provider is configured, the built-in local model is used.
        If an external provider fails or returns no usable suggestions, the local
        model is used as a safe fallback.
        """
        local = self._recommend_words_local(context_words, profile, prefix)
        if self.suggestion_provider is None:
            return local

        prompt = self._prompt_so_far(context_words, prefix)
        try:
            external = self.suggestion_provider.get_word_suggestions(
                prompt=prompt,
                locale=self.suggestion_locale,
                limit=self.word_prediction_slots,
            )
        except (OSError, urllib.error.URLError, TimeoutError, ValueError, json.JSONDecodeError):
            external = []

        suggestions = self._dedupe(external, self.word_prediction_slots)
        if self.fill_external_suggestions and len(suggestions) < self.word_prediction_slots:
            suggestions = self._dedupe(suggestions + local, self.word_prediction_slots)
    ...
    def visible_letters(self, context_words: List[str], profile: SyntheticUserProfile, prefix: str = "") -> List[str]:
        """Return visible next-letter suggestions from the selected backend."""
        local = self._visible_letters_local(context_words, profile, prefix)
        if self.suggestion_provider is None:
            return local

        prompt = self._prompt_so_far(context_words, prefix)
        try:
            external = self.suggestion_provider.get_letter_suggestions(
                prompt=prompt,
                locale=self.suggestion_locale,
                limit=self.visible_letter_slots,
            )
        except (OSError, urllib.error.URLError, TimeoutError, ValueError, json.JSONDecodeError):
            external = []

        suggestions = self._dedupe([c[:1] for c in external if c and c[:1].isalpha()], self.visible_letter_slots)
        if self.fill_external_suggestions and len(suggestions) < self.visible_letter_slots:
            suggestions = self._dedupe(suggestions + local, self.visible_letter_slots)
        return suggestions or local
